        ...
        {editingId ? (
          ...
          <div className="space-y-8">
            {Object.keys(groupedTemplates).length === 0 ? (
              ...
              // Fixed: Cast 'items' to 'Template[]' as Object.entries returns 'unknown' for values in some environments.
              Object.entries(groupedTemplates).map(([category, items]) => {
                const templateItems = items as Template[];
                return (
                  <div key={category} className="space-y-3">
                    <div className="flex items-center gap-2">
                      <h3 className="text-sm font-bold text-slate-400 uppercase tracking-wider">{category}</h3>
                      <div className="flex-1 h-px bg-slate-100"></div>
                      <span className="text-[10px] font-bold bg-slate-100 text-slate-500 px-2 py-0.5 rounded-full">{templateItems.length}</span>
                    </div>
